...
### やり方（3 つ。どれでも同じことをします）

| | やり方 |
|---|---|
| **Windows** | **`Reel Studio 更新.cmd` をダブルクリック** → 終わったら `Reel Studio.cmd` で起動し直す |
| **画面から** | Settings（Ctrl+6）の「版と更新」→「更新する」→ 起動し直す |
| **コマンド** | `npm run update` → 起動し直す |

更新があるときは、起動したときの黒い画面にも `! 更新があります（N 件）` と出ます。

### はじめて更新するときだけ（0.1.0 を使っている人）

0.1.0 には上の仕組みがまだ入っていません。**1 回だけ**次を実行してください。

```bash
git pull
```

そのあと `Reel Studio.cmd` で起動し直せば終わりです（依存パッケージは変わっていないので
`npm install` は要りません。画面は起動時に自動で作り直されます）。
次からは上の 3 つのやり方が使えます。
