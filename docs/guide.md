...
**三点リーダーは全角 3 文字の「・・・」に固定する**（2026-09-23 のユーザー規則。縦書きで「…」は細く見えて消える）。
プロンプトで指示するだけでなく、テロップを書き出す場所（`scriptPlanToCuts` / `renderMimicScript` / テロップ生成 /
編集の差分 / フック差し替え）で `shared/telop-text.ts` の `normalizeEllipsis` を必ず通し、「…」「……」「...」「‥」「･･･」「・・」を
「・・・」に揃える。手で入れたテロップは検証の `TELOP_ELLIPSIS_FORM`（W）が拾い、Timeline の修正ボタン（`setText`）で直せる。
